...
1) as a side product "tweened" or interpolated images between two given files can also be generated if this is useful in follow-up processing in Video Sequence Editors etc.

Step 2 uses OpenCV (https://opencv.org/) for basic image processing.

This is by no means intended as any form of tutorial and not a proper way to do things with OpenCV.
...
It may serve however for a future me as a reference for my thought process and might help someone starting with image processing to check the thought process of others.
...
Each process step has a output directory
...
- 04_cropped_imgs  svgcrop.py can convert a svg image with a path from 03_shape_verified_imgs  into pngs where the paths shape is cropped out from the rest of the image and the "background" blackened out/set to transparent
...
The frames parameter defines how many frames are generated in the transition of one image to the next. Still-frames defines how often the last frame is repeated. For the animation loop shown in the gif-file above both values were set to 60 and the rendering software was set to 30fps.
...
Computer vision and related tech like "AI" based image processing (in most cases neural networks at the end of the day) are all the buzz at the moment. So when I was faced with the intention of "finding a roughly rectangular drawing in a picture" I was expecting to find something that works out of the box. In the end we have self-driving cars and dreaming AIs producing images of [pigsnails](https://photos.google.com/share/AF1QipPX0SCl7OzWilt9LnuQliattX4OUCj_8EP65_cTVnBmS1jnYgsGQAieQUc1VQWdgQ?key=aVBxWjhwSzg2RjJWLWRuVFBBZEN1d205bUdEMnhB), so this should be a trivial one. Well it seems there are no "out of the box" solutions just yet.

So I tried to take a crack at it by using OpenCV and unfortunately the results were still rather disappointing:

![Sample detection using shape detection of OpenCV](./readmefiles/tweet_0027_400.png "Sample detection using shape detection of OpenCV")
...
![Example greyblacked](./readmefiles/2019_02_12_GREYBLACKED.png "Example greyblacked")

It quickly becomes obvious that this will fail in most cases with bad results, because the figures of the image are colored and will therefore be blackened out. Growing from the inside out will hit the "black wall" of the character on the right. 
...
Note that the algorithm still does all the calculation on the black and white image from before - but having it shown on the original colored image is well... more "visually appealing".
...
The result of this process leaves us with 4 edges which corners give us the result below (blue line). Green is the assumed inner part of the image (4% offset) and red is a (7% offset) both are not used anymore in further processing but they look fancy :). 
...
We can do better for chisel! 
---------------------------

Out of the rough block of marble cut out from the raw picture we take our chisel and craft something useful (shrinkoptcut.get_optimized_rect). 
...
(For the serious people who don't get the pun in the title of this section and lived under a rock for the last 20 years: It is a play on the slang "forshizzle ma nizzle" (google E-40  and Snoopdog!) and a reference to the hammer and chisel analogy before - yes I am kinda counting on the fact that you might have read all this from top to bottom... which is a long shot - I know - but you have to aim for something right?)
...
On the way from pure OpenCV approach to the shrinkbox approach I tried to create what I call "grey smear images" which at the end of the day led to nowhere but came in handy now. It calculates the average pixel value by summing the average pixel value of the line and column the pixel is in.
...
The implementation of the perspectively (is that even an English word?) correct projection with size estimation is not my own
